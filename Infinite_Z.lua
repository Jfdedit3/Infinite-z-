...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Infinite Z is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end

currentVersion = "0.1 (BETA_TEST)"
...
Title.TextSize = 18
Title.Text = "Infinite Z v" .. currentVersion
...
StayOpen.Text = "Keep Open/ laisser ouvert"
...
PluginsHint.Text = "Download plugins from the IZ Discord (discord.gg/SQpfRUBfB7)"
...
selectChat.Text = "Chat Logs/clogs"
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with IZ,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			toClipboard("https://discord.gg/SQpfRUBfB7")
		...
		inviteButton.Text = "Copy Discord Invite Link (https://discord.gg/SQpfRUBfB7)"
	end)
	dragGUI(main)
	main.Parent = PARENT
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Infinite Z Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Infinite Z support server.'}
...
CMDs[#CMDs + 1] = {NAME = 'hideiy', DESC = 'Hides the main IZ GUI'}
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		toClipboard('https://discord.com/invite/SQpfRUBfB7')
		notify('Discord Invite', 'Copied to clipboard!\ndiscord.gg/SQpfRUBfB7')
	else
		notify('Discord Invite', 'discord.gg/SQpfRUBfB7')
	end
	if httprequest then
		httprequest({
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Infinite Z may have issues or features that are not functioning correctly.") end
end)
